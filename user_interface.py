...
def ok_command():
    ...
    try:
        youtube_video = downloader.video(entry.get(), on_progress_callback=on_progress, on_complete_callback=finish_download)
    except exceptions.RegexMatchError:
        if not entry.get() == 'thank you':
            messagebox.showerror('Unvalid URL!', 'The url you entered is invalid. Please make sure you typed the url correctly.')
        else:
        ...
        messagebox.showerror('Video Unavailable', 'The video you want to download cannot be found. Video may be age restricted, removed from your region, members only, or private.')
        return False
    except Exception:
...
def loading_animation():
    while loading:
        ...
        if not loading:
            ...
            file_size.set(str(int(stream.get_highest_resolution().filesize_mb))+' MB')
            list_resolutions()
...
def get_file_size(value):
    file_size.set(str(int(videos[video_resolutions.index(value)].filesize_mb))+' MB')
...
def finish_download(a, b):
    messagebox.showinfo('Download Completed', 'Your video has been downloaded successfully. Video location: \n'+str(b))
...
def download_command():
    ...
    w = 430
    h = 96
    progress_window.geometry(f'{w}x{h}+{int((root.winfo_screenwidth()-w)/2)}+{int((root.winfo_screenheight()-h)/2)}')

    label_frame = ctk.CTkFrame(master=progress_window, fg_color='transparent', border_width=0)
    download_label = ctk.CTkLabel(master=label_frame, text='Downloading Video:')
    bytes_label = ctk.CTkLabel(master=label_frame, textvariable=bytes_completed)
    progress_bar = ctk.CTkProgressBar(master=progress_window, width=400, height=10, progress_color='#00FF00', determinate_speed=0.03)
    download_ok_button = ctk.CTkButton(master=progress_window, text='OK', state='disabled', command=progress_window.destroy)
    progress_bar.set(0)
    label_frame.grid(row=0, column=0, columnspan=2,padx=10, pady=5)
    ...
    downloading_thread = threading.Thread(target=video.download, kwargs={'output_path': path}, daemon=True)
...
url = ctk.StringVar()
url.set('https://www.youtube.com/watch?v=a3Kd4MRaRAw&ab_channel=GitHub')
title = ctk.StringVar()
channel = ctk.StringVar()
length = ctk.StringVar()
file_size = ctk.StringVar()
loading_text = ctk.StringVar()
...
title.set(first_video.title)
channel.set(first_video.author)
length.set(first_video.length_str())
photo = first_video.thumbnail_image()

# Information Section
thumbnail_label = tk.Label(master=information, image=photo)
thumbnail_label.image = photo
title_label = ctk.CTkLabel(master=information, textvariable=title, font=('Roboto', 14), justify=tk.LEFT, wraplength=190)
channel_label = ctk.CTkLabel(master=information, textvariable=channel, font=('Roboto', 12))
length_label = ctk.CTkLabel(master=information, textvariable=length, font=('Roboto', 12))

# Information Grid
thumbnail_label.grid(row=0, column=0, rowspan=2, padx=(12, 5), pady=10, sticky=tk.N+tk.W)
title_label.grid(row=0, column=1, columnspan=2, padx=(5, 15), pady=13, sticky=tk.N+tk.W)
...
# Button Section 
button_frame = ctk.CTkFrame(master=main_window, fg_color='transparent', border_width=0)
ok_button = ctk.CTkButton(master=button_frame, text='OK', command=ok_command)
continue_button = ctk.CTkButton(master=button_frame, text='Continue', state='disabled', command=continue_download)
theme_button = ctk.CTkOptionMenu(master=button_frame, values=['Light Mode', 'Dark Mode'], anchor=ctk.CENTER, command=set_theme)
...
directory_button = ctk.CTkButton(master=download_frame, text='Select Folder', command=set_directory)
resolutions_menu = ctk.CTkOptionMenu(master=download_frame, values=['144p', '240p', '360p', '480p', '720p'], anchor=ctk.CENTER, state='disabled', command=get_file_size)
file_size_label = ctk.CTkLabel(master=download_frame, textvariable=file_size)
download_button = ctk.CTkButton(master=download_frame, text='Download', state='disabled', command=download_command)

# Grids
entry.grid(row=1, column=0, columnspan=5, padx=10, pady=(10, 5), sticky=tk.E+tk.W)
information.grid(row=2, column=0, rowspan=2, columnspan=3, padx=10, pady=5, sticky=tk.W+tk.N+tk.E)
button_frame.grid(row=2, column=4, padx=10, pady=5, sticky=tk.N+tk.E)
...
download_frame.grid(row=4, column=0, columnspan=5, padx=10, pady=5, sticky=tk.E+tk.W)
