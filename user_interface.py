...
def ok_command():
    ...
    try:
        youtube_video = downloader.video(entry.get(
        ), on_progress_callback=on_progress, on_complete_callback=finish_download)
    except exceptions.RegexMatchError:
        if not entry.get() == 'thank you':
            messagebox.showerror(
                'Unvalid URL!', 'The url you entered is invalid. Please make sure you typed the url correctly.')
        else:
...
thumbnail_label.image = photo
title_label = ctk.CTkLabel(master=information, textvariable=title, font=(
    'Roboto', 14), justify=tk.LEFT, wraplength=190)
...
resolutions_menu = ctk.CTkOptionMenu(master=download_frame, values=[
